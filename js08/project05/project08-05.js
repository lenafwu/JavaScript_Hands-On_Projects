...
function showNextBoard(game) {
  ...
  function moveCell(start, end) {
    document
      .getElementById(end)
      .appendChild(document.getElementById(start).firstElementChild);
  }
  ...
  function pawnPromotion() {
    ...
    switch (newPiece) {
      case "P":
        rankNum = 9817;
        break; // unicode for white pawn
      case "N":
        rankNum = 9816;
        break; // unicode for white knight
      case "B":
        rankNum = 9815;
        break; // unicode for white bishop
      case "R":
        rankNum = 9814;
        break; // unicode for white rook
      case "Q":
        rankNum = 9813;
        break; // unicode for white queen
      case "K":
        rankNum = 9812;
        break; // unicode for white king
    }
    if (game.move % 2 === 1) {
      // move was made by black change unicode to black piece image
      rankNum += 6;
    }

    // Change image to promoted piece
    document.getElementById(endCell).firstElementChild.innerHTML =
      "&#" + rankNum + ";";
  }
...
function showPrevBoard(game) {
  ...
  function addCell(cell) {
    // Moves a captured piece to the box
    if (game.move % 2 === 0) {
      // move the black piece back to the board
      document.getElementById(cell).appendChild(blackBox.lastElementChild);
    } else {
      // move the white piece back to the board
